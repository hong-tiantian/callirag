from typing import Optional  # [ADAPTER] for Optional[dict] retrieval_inputs type hint
...
class StyleRSIUpBlock2D(nn.Module):
    def __init__(
        ...
        self.gradient_checkpointing = False
        self.retrieval_adapter = None  # [ADAPTER] attach point, 默认关闭
    ...
    def forward(
        ...
        encoder_hidden_states=None,
        upsample_size=None,
        retrieval_inputs: Optional[dict] = None,  # [ADAPTER] optional retrieval conditioning
    ):
        total_offset = 0

        style_content_feat = style_structure_features[-self.upblock_index-2]
        # [ADAPTER-BEGIN] one-shot inject before offset-path loop
        if self.retrieval_adapter is not None and retrieval_inputs is not None:
            delta = self.retrieval_adapter(h_q=style_content_feat, **retrieval_inputs)
            style_content_feat = style_content_feat + delta
        # [ADAPTER-END]
